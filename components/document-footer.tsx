 ...
 */
export function DocumentFooter({ webring, prev, next, backLinks }: DocumentFooterProps) {
  return (
    <>
      {(prev || next) && (
      ...
      <div className="foot">
        {webring && (
          <div className="webring">
            ...
            <br />
            <Link href="#">[ &lt; Prev ]</Link> &nbsp; <Link href="#">[ Random ]</Link> &nbsp;{" "}
            <Link href="#">[ List ]</Link> &nbsp; <Link href="#">[ Next &gt; ]</Link>
          </div>
        )}
        <div className="badges">
          {BADGES.map((badge) => (
            <div key={`${badge.a}-${badge.b}`} className={`badge ${badge.cls}`}>
              <span className={`${"big" in badge && badge.big ? "b1 " : ""}${"mono" in badge && badge.mono ? "mono" : ""}`.trim()}>
                {badge.a}
              </span>
              <span>{badge.b}</span>
            </div>
        ...
        <div className="legal">
          Copyright © 1998–{buildInfo.commitYear} Mason Matich (KC3WNY). All rights reserved.
          &nbsp;·&nbsp; 73 de KC3WNY
        </div>
      </div>
    </>
