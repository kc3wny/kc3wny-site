...
export function ExperienceSection({ num, workData, projectData }: ExperienceSectionProps) {
  return (
    <>
      <SectionHeading num={num} title="Experience & Leadership" id="experience" />

      <h3 className="sub">Work Experience</h3>
      {workData.map((job, idx) => (
        <div key={`${job.role}-${idx}`} className="entry">
      ...
      {projectData.map((project, idx) => (
        <div key={`${project.name}-${idx}`} className="entry">
